...
const formatPersonName = (person) => {
  ...
  const displayName = person.name || person.full_name || "";
...
export default function FacultySubjectLoad({
  availableSubjects = [],
  enrollment = {},
  creditedSubjects = [],
  preselectedSubjects = [],
  ...
  const page = usePage();
  const { facultyEvaluationAccess = {} } = page.props || {};
  const canPrintCorAccess = facultyEvaluationAccess?.canPrintCor;
  ...
  const corEvaluatorName = useMemo(() => {
    if (authUser) {
      const authDisplay = formatPersonName(authUser) || authUser.name || authUser.email;
      if (authDisplay) {
        return authDisplay;
      }
    }
    ...
    return "N/A";
  }, [authUser, corStudentRecord?.evaluator, corStudentRecord?.processed_by]);
  ...
  const studentMetaDetails = useMemo(() => {
    ...
    const baseLine = `ID: ${enrollment.student.id_number || 'N/A'} • ${programCourseLabel || 'N/A'} • ${enrollment.year_level?.year_level || 'N/A'}`;
    return resolvedSectionLabel ? `${baseLine} • Section: ${resolvedSectionLabel}` : baseLine;
  }, [enrollment, resolvedSectionLabel, programCourseLabel]);
  ...
  const subjects = useMemo(() => {
    ...
    const completedSubjects = Array.isArray(enrollment?.completed_subjects)
    ...
    if (Array.isArray(enrollmentSubjects) && enrollmentSubjects.length > 0) {
      return enrollmentSubjects
        .map((entry, index) => {
          ...
          const curriculum =
            classSchedule.curriculum_subject ||
            classSchedule.curriculumSubject ||
            entry.curriculum_subject ||
            entry.curriculumSubject ||
            {};
          const subject = curriculum.subject || entry.subject || {};
  ...
  const handleSubjectLoad = useCallback(() => {
    ...
    const submissionRoute = route("faculty.evaluation.subjectload.store");
        ...
        {showSubjectListModal && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-5xl rounded-2xl border border-slate-200 bg-white shadow-xl"
            >
              ...
              <div className="px-5 py-4">
                ...
                <div className="max-h-[65vh] overflow-hidden rounded-2xl border border-slate-200">
          ...
          createPortal(
            ...
            >
              ...
              <motion.div
                ...
                className="bg-white w-full max-w-[720px] h-[90vh] rounded-lg p-6 relative overflow-y-auto"
              >
                <button
                  type="button"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="flex">
                        ...
                        <span className="text-gray-800">
                          {corCourseMajorLabel} {corStudentRecord?.year_level?.year_level || corStudentRecord?.year_level_name || ""}
                        </span>
                      </p>
                    </div>
                    <div className="space-y-1.5">
                ...
                <div className="text-[11px] text-gray-600 space-y-2 cor-card cor-footer">
                  <div>
                    <p className="font-semibold text-gray-800">Evaluated by: {corEvaluatorName}</p>
                ...
                <div className="mt-6 flex flex-wrap items-center justify-between gap-3 print-hidden">
                  <button
                    type="button"
                    onClick={() => router.visit(route('faculty.evaluation.enrollment'))}
                    className="flex items-center gap-2 rounded-full border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-100"
                  >
                    Back to Enrollment
                  </button>
                  {canPrintCorAccess && (
                    <button
                      ...
                      className="bg-indigo-600 text-white w-11 h-11 rounded-full text-xs font-medium shadow hover:bg-indigo-700 transition inline-flex items-center justify-center order-2"
                    >
                      🖨️
                    </button>
                  )}
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[11px] font-semibold text-emerald-600 shadow-sm transition hover:border-emerald-300 hover:text-emerald-700 order-3"
                  >
