...
export default function FacultyAttendanceReport({ summary = {}, recent = [] }) {
  ...
  const totalFiltered = useMemo(() => {
    if (statusFilter === "all") {
      return summary.total ?? filteredStatusEntries.reduce((acc, [, total]) => acc + (Number(total) || 0), 0);
    }
  ...
  const handleExportPDF = () => {
    ...
    doc.text("Faculty Attendance Report", 14, 16);
    autoTable(doc, {
      startY: 22,
    ...
    doc.save("faculty-attendance-report.pdf");
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(workbook, "faculty-attendance-report.xlsx");
  };

  return (
    <FacultyLayout>
      <Head title="Attendance Snapshot" />
      <div className="mx-auto w-full max-w-5xl space-y-5 px-4 py-6 text-[13px] text-slate-600">
        <header className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1 text-slate-700">
            <h1 className="text-xl font-semibold text-slate-900">Attendance Snapshot</h1>
            <p className="text-[11px] text-slate-500">Tracks attendance submissions across your assigned sections.</p>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-[11px] font-semibold text-slate-700">Filters</h2>
              ...
              {filteredStatusEntries.length > 0 ? (
                filteredStatusEntries.map(([status, total]) => (
                  <div
                    ...
                    className="flex items-center justify-between rounded border border-slate-100 bg-slate-50 px-3 py-1.5 text-[12px] text-slate-600"
                  >
